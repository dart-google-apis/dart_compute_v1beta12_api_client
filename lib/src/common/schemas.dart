part of compute_v1beta12_api_client;
...
class MachineType {
  ...
  MachineType.fromJson(Map json) {
    ...
    if (json.containsKey("hostCpus")) {
      hostCpus = json["hostCpus"];
    }
    if (json.containsKey("id")) {
      id = json["id"];
    }
...
class MetadataItems {
  ...
  /** Key for the metadata entry. Keys must conform to the following regexp: [a-zA-Z0-9-_]+, and be less than 128 bytes in length. This is reflected as part of a URL in the metadata server. Additionally, to avoid ambiguity, keys must be unique. */
  String key;

  /** Value for the metadata entry. These are free-form strings, and only have meaning as interpreted by the image running in the instance. The only restriction placed on values is that their size must be less than or equal to 15000 bytes. */
  String value;
